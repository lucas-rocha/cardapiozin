...
const MenuItemRequest_1 = require("@src/models/requests/MenuItemRequest");
const ApiError_1 = require("@src/util/errors/ApiError");
const validate_1 = require("@src/util/validate");
const S3Service_1 = require("./S3Service");
const pathImages_1 = require("@src/util/pathImages");
...
class MenuItemService {
    constructor(s3Service = new S3Service_1.S3Service(), categoryService = new CategoryService_1.CategoryService(), additionalItemService = new AdditionalItemService_1.AdditionalItemService(), prisma = PrismaClient_1.default.getInstance()) {
        this.s3Service = s3Service;
        this.categoryService = categoryService;
        this.additionalItemService = additionalItemService;
        this.prisma = prisma;
    }
    async createMenuItem(menuItem, file) {
        ...
        if (exist)
            throw new ApiError_1.ConflictError('Item name already exists!');
        await (0, validate_1.validateInputs)(menuItem, MenuItemRequest_1.MenuItemRequest);
        const result = await this.prisma.$transaction(async (transaction) => {
            ...
            const createdCategories = await this.categoryService.createCategories({ categoriesList: menuItem.categoriesCreated, restaurant_id: menuItem.restaurant_id }, 100, file ? file : null, transaction);
            ...
            const itemIds = addItems.reduce((acc, item) => {
                const current = menuItem.additionalItemCreated.find(x => x.name == item.name);
                if (current === null || current === void 0 ? void 0 : current.isAdditional) {
                    acc.additionalIds.push(item.id);
                }
                if (current === null || current === void 0 ? void 0 : current.isRemoved) {
                    acc.removeIds.push(item.id);
                }
                return acc;
            }, { additionalIds: [], removeIds: [] });
    ...
    async getMenuItem() {
        ...
        const result = categoriesWithProducts.map(category => ({
            ...
            itens: category.Menu_item_Category.map(menuItemCategory => ({
                ...
                image: menuItemCategory.Menuitem.image ? pathImages_1.paths.menuItemsPath + menuItemCategory.Menuitem.image : null
            }))
        }));
        return result;
    }
    async getMenuItemById(id) {
        ...
        if (!menuItem)
            throw new ApiError_1.NotFound('Menu item not found!');
        return {
            id: menuItem.id,
    ...
    async updateMenuItemById(menuItemUpdate, file) {
        ...
        if (menuItemUpdate.categoryIds.length == 0 && menuItemUpdate.categoriesCreated.length == 0)
            throw new ApiError_1.UnprocessableError('At least there should be a category');
        const result = await this.prisma.$transaction(async (transaction) => {
            ...
            const createdCategories = menuItemUpdate.categoriesCreated ? await this.categoryService.createCategories({ categoriesList: menuItemUpdate.categoriesCreated, restaurant_id: menuItemUpdate.restaurant_id }, 100, file ? file : null, transaction) : [];
    ...
    async deleteMenuItem(id) {
        ...
        if (!exists)
            throw new ApiError_1.NotFound("throw new NotFound");
        const result = await this.prisma.$transaction(async (transaction) => {
